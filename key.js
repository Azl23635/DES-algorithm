export class Key {
  ...
  two_bitRotate = (keyLeftRight56, round) => {
    ...
    for (let i = 0; i < bitShiftForRound; i++) {
      keyLeft = rotateKey(keyLeft);
      keyRight = rotateKey(keyRight);
    }
    output.push(keyLeft + keyRight);
    return output;
  };

  three_permutationTwo = (keyLeftRightRotated) => {
    let pcTwoOutput48 = '';
    const pcTwoLookup = [
  ...
  roundKeyGen = (key64, round) => {
    return three_permutationTwo(
      two_bitRotate(one_dropThenPermute(key64), round)
    );
  };
}
