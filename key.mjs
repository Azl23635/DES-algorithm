export default class Key {
  //Function shifts the bit by one (to left)
  ...
  static #one_dropThenPermute = (key64) => {
    let left = '';
    let right = '';
    //Lookup table for first permutation
    let pcLookup = [
      [
    ...
    //Order bits according to lookup table
    ...
    //Return the left and right sides
    return [left, right];
  };
  ...
  static #two_bitRotate = (keyLeftRight56, round) => {
    ...
    //Lookup bitshift for every round
    const bitShiftLookup = [
      1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1,
    ];
    const output = [];
    //Adds up the num of bit shift required for current round
    const bitShiftForRound = bitShiftLookup
      .slice(0, round)
      .reduce((total, curr) => total + curr, 0);
    //Bit shift by required amount for both left/right sides
    for (let i = 0; i < bitShiftForRound; i++) {
      keyLeft = this.#rotateKey(keyLeft);
      keyRight = this.#rotateKey(keyRight);
    }
    //Concatenate the keys together to return the bit shifted key
  ...
  static #three_permutationTwo = (keyLeftRightRotated) => {
    let pcTwoOutput48 = '';
    //Lookup table for permutation two
    const pcTwoLookup = [
    ...
    //Do the permutation, return result
